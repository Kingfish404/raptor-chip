        ...
        switch (flag)
        {
            case 0:
                ...
                retval = core_bench_state(res->size,
                                          ...
                                          dtype,
                                          res->core_crc);
                if (res->core_crcstate == 0)
                    res->core_crcstate = retval;
                break;
            case 1:
                retval = core_bench_matrix(&(res->mat), dtype, res->core_crc);
                if (res->core_crcmatrix == 0)
                    res->core_crcmatrix = retval;
                break;
            default:
                retval = data;
                break;
        }
        res->core_crc = core_crcu16(retval, res->core_crc);
        retval &= 0x007f;
...
/* Benchmark for linked list:
        ...
        - List sort
        - Operate on data from list (core_crc)
...
core_bench_list(core_results *res, ee_s16 finder_idx)
{
    ...
    /* core_crc data content of list from location of index N forward, and then undo
     * remove */
    ...
    while (finder)
    {
        retval = core_crc16(list->info->data16, retval);
        finder = finder->next;
    }
#if CORE_DEBUG
