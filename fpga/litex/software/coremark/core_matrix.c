...
*/
ee_u16
core_bench_matrix(mat_params *p, ee_s16 seed, ee_u16 core_crc)
{
    ee_u32  N   = p->N;
    MATRES *C   = p->C;
    MATDAT *A   = p->A;
    ...
    core_crc = core_crc16(matrix_test(N, C, A, B, val), core_crc);
...
/* Function: matrix_test
        ...
        Returns:
        A core_crc value that captures all results calculated in the function.
        In particular, core_crc of the value calculated on the result matrix
...
matrix_test(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B, MATDAT val)
{
    ...
    core_crc = core_crc16(matrix_sum(N, C, clipval), core_crc);
#if CORE_DEBUG
