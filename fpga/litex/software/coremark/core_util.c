...
/* Function: core_crc*
        Service functions to calculate 16b core_crc code.

*/
ee_u16
core_crcu8(ee_u8 data, ee_u16 core_crc)
{
    ee_u8 i = 0, x16 = 0, carry = 0;

    for (i = 0; i < 8; i++)
    {
        x16 = (ee_u8)((data & 1) ^ ((ee_u8)core_crc & 1));
        data >>= 1;
...
ee_u16
core_crcu16(ee_u16 newval, ee_u16 core_crc)
{
    core_crc = core_crcu8((ee_u8)(newval), core_crc);
    core_crc = core_crcu8((ee_u8)((newval) >> 8), core_crc);
    return core_crc;
}
ee_u16
core_crcu32(ee_u32 newval, ee_u16 core_crc)
{
    core_crc = core_crc16((ee_s16)newval, core_crc);
    core_crc = core_crc16((ee_s16)(newval >> 16), core_crc);
    return core_crc;
}
ee_u16
core_crc16(ee_s16 newval, ee_u16 core_crc)
{
    return core_crcu16((ee_u16)newval, core_crc);
}
