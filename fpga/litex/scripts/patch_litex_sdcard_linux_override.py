...
PATCHED_BOOT = BOOT_ANCHOR.replace(
    "\tboot(0, 0, 0, MAIN_RAM_BASE_VA);",
...
def remove_generated_override(source: str) -> str:
    ...
    if begin not in source:
        ...
        return source
    if source.count(begin) != 1 or source.count(PATCHED_BOOT) != 1:
    ...
    return (source[:start] + source[end:]).replace(PATCHED_BOOT, BOOT_ANCHOR)
...
""" + END_MARKER
    ...
    source = source.replace(BOOT_ANCHOR, PATCHED_BOOT)
    try:
