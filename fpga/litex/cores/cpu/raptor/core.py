...
GCC_FLAGS = {
    "standard": "-march=rv32imac_zicsr_zifencei_zba_zbb_zbc_zbs -mabi=ilp32",
    "linux":    "-march=rv32imac_zicsr_zifencei_zba_zbb_zbc_zbs -mabi=ilp32",
}
...
class Raptor(CPU):
    ...
    def add_sources(platform, variant="standard"):
        raptor_home = os.environ.get("RAPTOR_HOME", os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", ".."))
        ...
        if not os.path.exists(pack_sv):
            ...
            ret = os.system(f"make -C {os.path.join(raptor_home, 'nsim')} pack {vflags}")
            if ret != 0:
