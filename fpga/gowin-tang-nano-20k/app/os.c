...
static char *hello = "Hello from RISC-V chip\r\n  type any key to continue\r\n\r\n";
static volatile char data[128];
...
void run()
{
  ...
  uart_send_str("mvendorid: ");
  uart_send_hex_byte((char)(mvendorid >> 24));
  uart_send_hex_byte((char)(mvendorid >> 16));
  uart_send_hex_byte((char)(mvendorid >> 8));
  uart_send_hex_byte((char)(mvendorid));
  uart_send_str("\r\nmarchid:   ");
  uart_send_hex_byte((char)(marchid >> 24));
  uart_send_hex_byte((char)(marchid >> 16));
  uart_send_hex_byte((char)(marchid >> 8));
  uart_send_hex_byte((char)(marchid));
  uart_send_str("\r\n");

  uart_send_str(hello);
  uart_read_char();
  uart_send_str("  type 'help'\r\n");
