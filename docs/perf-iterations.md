...
| current | L2$ + Larger L1$ [11]   | 15528120 | 15914408 | 1.025 | 2e6,14 | 6e6,39    | 9e6, 57    | 2e5, 2 | 0e0, 0 | 5e6, 29   |
...
| current | L2$ + Larger L1$ [11]   | 448103 | 339931 | 0.759 | 8e4,19 | 1e5,26    | 2e5, 47    | 2e4, 4 | 0e0, 0 | 2e5, 46   |
...
[11]: L2 cache (`rapt_l2.sv`) + L1I/SRAM refactor, plus EXU/MUL and typo/lint cleanups. The dominant benchmark gain is in the frontend: CoreMark IFU stall drops from 26% to 14% (L1I miss path + SRAM pre-read tuning), pulling CoreMark IPC **0.938->1.025 (+9.3%)**, -1430585 cycles, dual commit 44.5%, BPU 92.6%. MicroBench IPC 0.740->0.759 (+2.6%, -11427 cycles, dual commit 39.8%, BPU 78.9%); its working set fits in L1 so the L2 is rarely exercised and gains come mainly from the same frontend tuning. #Inst essentially unchanged (CoreMark identical, MicroBench +24). STA below is run with the `small` (single-issue) uarch preset.
...
From `[11]` onward, STA is run with the `small` (single-issue, low-area) uarch preset (`make sta RAPT_CONFIG=small`). Earlier rows (`[9]`, `[10]`) used the `default` (dual-issue OoO) preset, so `[11]` is not directly comparable in area/Fmax.
...
| current | L2$ + L1I/SRAM [11]   |     77.15 |   529,830.50 | 5.18e-02 |      94.79 |  49,517.43 |  1.22e-02 |       32.89 | 2,818,931.08 |   3.12e-02 |
...
| current | L2$ + L1I/SRAM [11]   |     53.78 |   859,590.97 | 9.00e-02 |      57.67 |  80,248.39 |  6.80e-03 |       22.71 | 4,680,833.04 |   5.09e-02 |

Footnote `[9]` matches the corresponding entry in the CoreMark/MicroBench tables above. `[10]` and `[11]` STA were run via CI across all three PDKs and both ISAs. `[10]` used the `default` (dual-issue OoO) preset; `[11]` switches to the `small` (single-issue) preset, so its lower area/higher Fmax (e.g. RV32 NG45 area 1.04M -> 0.53M um^2, Fmax 62.08 -> 77.15 MHz) reflect the smaller preset, not a like-for-like uarch comparison.
