...
- Config: unless a row or footnote says otherwise, benchmark rows use the historical two-wide NPC path. Current default RTL/config independently declares `DECODE/RENAME/DISPATCH/COMMIT_WIDTH=2/2/2/2`, `ROB_SIZE=32`, `RS_SIZE=8`, `RIQ/IIQ=8`, `IOQ_SIZE=8`, `SQ_SIZE=16`, `PHY_SIZE=128`, L1I and L1D each 8 KiB (32 sets × 4 ways, 64-byte lines), TAGE direction predictor, and `RAPT_L2_EN` disabled by default (the instantiated L2 stage is passthrough unless that macro is defined). These current settings do not retroactively describe the historical measurements below.
