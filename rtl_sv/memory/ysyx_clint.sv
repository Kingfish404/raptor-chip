...
module ysyx_clint #(
  ...
  always_comb begin
    out_rdata = '0;
    case (araddr)
      `YSYX_CLINT_MSIP:     out_rdata = {{(XLEN - 1) {1'b0}}, msip_reg};
      `YSYX_CLINT_MTIMECMP: out_rdata = mtimecmp[XLEN-1:0];
      `YSYX_CLINT_MTIMECMP_UP: begin
        if (XLEN == 32) out_rdata = mtimecmp[63:32];
      end
      `YSYX_BUS_RTC_ADDR:   out_rdata = mtime[XLEN-1:0];
  ...
  always_ff @(posedge clock) begin
    if (reset) begin
      ...
      if (wvalid) begin
        case (awaddr)
          `YSYX_CLINT_MSIP: msip_reg <= wdata[0];
          `YSYX_CLINT_MTIMECMP: mtimecmp[XLEN-1:0] <= wdata;
          `YSYX_CLINT_MTIMECMP_UP: begin
