...
module ysyx_exu_ioq #(
  ...
  always_comb begin
    for (bit [XLEN-1:0] i = 0; i < IOQ_SIZE; i++) begin
      if (ioq_atom[i]) begin
        case (ioq_alu[i])
          `YSYX_ATO_LR__: ioq_data[i] = 'b0;
          `YSYX_ATO_SC__: ioq_data[i] = ioq_vk[i];
          `YSYX_ATO_SWAP: ioq_data[i] = ioq_vk[i];
          `YSYX_ATO_ADD_: ioq_data[i] = ioq_vk[i] + exu_lsu.rdata;
          `YSYX_ATO_XOR_: ioq_data[i] = ioq_vk[i] ^ exu_lsu.rdata;
          `YSYX_ATO_AND_: ioq_data[i] = ioq_vk[i] & exu_lsu.rdata;
          `YSYX_ATO_OR__: ioq_data[i] = ioq_vk[i] | exu_lsu.rdata;
          `YSYX_ATO_MIN_:
          ioq_data[i] = $signed(exu_lsu.rdata) < $signed(ioq_vk[i]) ? exu_lsu.rdata : ioq_vk[i];
          `YSYX_ATO_MAX_:
          ioq_data[i] = $signed(exu_lsu.rdata) > $signed(ioq_vk[i]) ? exu_lsu.rdata : ioq_vk[i];
          `YSYX_ATO_MINU: ioq_data[i] = exu_lsu.rdata < ioq_vk[i] ? exu_lsu.rdata : ioq_vk[i];
          `YSYX_ATO_MAXU: ioq_data[i] = exu_lsu.rdata > ioq_vk[i] ? exu_lsu.rdata : ioq_vk[i];
  ...
  always_comb begin
    for (int i = 0; i < IOQ_SIZE; i++) begin
      ioq_load_issue_vec[i] = ioq_valid[i] && ioq_ren[i]
          ...
          && (csr_bcast.dmmu_en
              || ysyx_pkg::addr_cacheable(ioq_eff_addr[i]) ||
  ...
  assign exu_lsu.rvalid = (issue_valid_now
      ...
      && (csr_bcast.dmmu_en
          || ysyx_pkg::addr_cacheable(
      exu_lsu.raddr
  ) || (active_idx == ioq_head && ioq_at_rob_head)));
  assign exu_lsu.raddr = ioq_atom[active_idx]
      ? ioq_vj[active_idx]
      : ioq_vj[active_idx] + ioq_imm[active_idx];
  assign exu_lsu.ralu = ioq_atom[active_idx] ? `YSYX_ALU_LW__ : ioq_alu[active_idx][4:0];
  assign exu_lsu.atomic_lock = ioq_atom[active_idx] && ioq_alu[active_idx] == `YSYX_ATO_LR__;
  ...
  always_comb begin
    case (ioq_alu[ioq_head])
      `YSYX_ATO_LR__: head_amo_wdata = 'b0;
      `YSYX_ATO_SC__: head_amo_wdata = ioq_vk[ioq_head];
      `YSYX_ATO_SWAP: head_amo_wdata = ioq_vk[ioq_head];
      `YSYX_ATO_ADD_: head_amo_wdata = ioq_vk[ioq_head] + head_rdata;
      `YSYX_ATO_XOR_: head_amo_wdata = ioq_vk[ioq_head] ^ head_rdata;
      `YSYX_ATO_AND_: head_amo_wdata = ioq_vk[ioq_head] & head_rdata;
      `YSYX_ATO_OR__: head_amo_wdata = ioq_vk[ioq_head] | head_rdata;
      `YSYX_ATO_MIN_:
