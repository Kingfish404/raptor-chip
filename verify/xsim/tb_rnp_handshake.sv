...
module tb_rnp_handshake;
  ...
  initial begin
    ...
    cpu.wvalid=0;
    cpu.bready=0;
    cpu.araddr='h20000000;
    cpu.arid=5;
    cpu.arlen=0;
    cpu.arsize=2;
    cpu.arburst=0;
    cpu.awaddr='h80000000;
    cpu.awid=7;
    cpu.awlen=0;
    cpu.awsize=2;
    cpu.awburst=0;
    ...
    tick(2);
    repeat (4) begin
      check(mem.wvalid && mem.wlast && mem.wdata == 'h12345678,
            "RNP W payload/last unstable under stall");
      tick(1);
    end
    mem.wready = 1;
    tick(1);
    ...
    tick(2);
    repeat (4) begin
      check(cpu.bvalid && cpu.bid == 7, "RNP lost accepted AW ID");
      tick(1);
    end
    cpu.bready = 1;
    tick(1);
    ...
    cpu.arid=10;
    for (int i = 0; i < 20; i++) begin
      if (mem.arvalid) break;
      tick(1);
    end
    check(mem.arvalid, "RNP downstream AR missing");
    tick(1);
    mem.arready=0;
    mem.rvalid=1;
    tick(2);
    repeat (4) begin
      check(cpu.rvalid && cpu.rlast && cpu.rid == 5 && cpu.rdata == 'habcdef01,
            "RNP R ownership/last unstable under stall");
      tick(1);
    end
    cpu.rready = 1;
    tick(1);
  ...
  initial begin
    #10000;
    $fatal(1, "RNP handshake watchdog");
  end
endmodule
