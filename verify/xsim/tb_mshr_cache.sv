...
module tb_mshr_cache;
  ...
  string mshr_trace;
  initial
    if ($value$plusargs("MSHR_RSP_TRACE=%s", mshr_trace)) begin
      $dumpfile(mshr_trace);
      $dumpvars(0, tb_mshr_cache);
    end
  int requests = 0;
  ...
  always @(posedge clock)
    if (!reset && l1d_bus.arvalid && l1d_bus.rready) begin
      if (!l1d_bus.ar_mshr || l1d_bus.ar_ptw || l1d_bus.arlen != 8'(Words - 1))
        $fatal(
            1,
            "expected tagged line request: mshr=%b ptw=%b len=%d eligible=%b safe=%b allowed=%b atomic=%b misaligned=%b",
            l1d_bus.ar_mshr,
            l1d_bus.ar_ptw,
            l1d_bus.arlen,
            dut.mshr_eligible,
            dut.refill_safe,
            lsu_l1d.replay_allowed,
            dut.l1d_atomic_lock,
            dut.l1d_orig_misaligned
        );
  ...
  task automatic refill(input int id, input int bad_word = -1, input int first_word = 0);
    ...
    outstanding[id] = 0;
  endtask
  task automatic response_stage_probe(input bit cancel);
    logic [XLEN-1:0] addr;
    int id;
    reset = 1;
    init_l1d_inputs();
    ...
    l1d_bus.rready = 1;
    addr = cancel ? XLEN'('h80001100) : XLEN'('h80001000);
    lookup(addr, 1);
    ...
    lsu_l1d.raddr  = addr;
    lsu_l1d.ralu   = XLEN == 64 ? `RAPT_ALU_LD__ : `RAPT_ALU_LW__;
    for (int n = 0; n < 20 && !dut.mshr_complete; n++) @(negedge clock);
    if (!dut.mshr_complete || lsu_l1d.rready || lsu_l1d.rdata != '0)
      $fatal(1, "MSHR response bypassed the local register");
    @(negedge clock);
    if (!lsu_l1d.rready || lsu_l1d.rdata != value(addr))
      $fatal(1, "registered MSHR response lost ready or data");
    if (!cancel) begin
      lsu_l1d.rvalid = 0;
      #1;
      if (lsu_l1d.rready) $fatal(1, "paused owner accepted a buffered MSHR response");
      repeat (2) @(negedge clock);
      if (dut.l1d_state != 3'b110 || dut.mshr_rsp_data_q != value(addr))
        $fatal(1, "MSHR response did not remain buffered during backpressure");
      lsu_l1d.rvalid = 1;
      #1;
      if (!lsu_l1d.rready) $fatal(1, "buffered MSHR response did not resume");
    end else begin
      cmu_bcast.flush_pipe = 1;
      #1;
      if (lsu_l1d.rready) $fatal(1, "flush did not cancel registered MSHR response");
    end
    @(negedge clock);
    lsu_l1d.rvalid = 0;
    cmu_bcast.flush_pipe = 0;
    if (lsu_l1d.rready) $fatal(1, "MSHR response remained valid after ownership ended");
  endtask
  initial begin
    init_l1d_inputs();
    if ($test$plusargs("MSHR_RSP_ONLY")) begin
      response_stage_probe(0);
      response_stage_probe(1);
      $display("PASS: minimal registered MSHR response and flush RV%0d", XLEN);
      $finish;
    end
    lsu_l1d.rvalid_b = 0;
    ...
    lsu_l1d.wvalid = 1;
    lsu_l1d.waddr  = XLEN'('h80000300);
    lsu_l1d.walu   = 8'((1 << (XLEN / 8)) - 1);
    lsu_l1d.wdata  = XLEN'(77);
    #1;
    ...
    begin
      ...
      l1d_bus.r_mshr = 1;
      l1d_bus.r_mshr_id = 2'(id);
      l1d_bus.rdata = value(issued_addr[id]);
      l1d_bus.rlast = 0;
      @(negedge clock);
      l1d_bus.rvalid = 0;
      l1d_bus.r_mshr = 0;
    ...
    $display(
        "PASS: MSHR cache RV%0d two in-flight lines, merge, full-table replay, reverse responses, flush drain, beat errors",
        XLEN);
    $finish;
  end
  initial begin
    #200000;
    $fatal(1, "MSHR cache timeout");
  end
endmodule
