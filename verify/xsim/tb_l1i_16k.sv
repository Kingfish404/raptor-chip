...
module tb_l1i_16k;
  ...
  initial begin
    ...
    if (`RAPT_CACHE_LINE_BYTES != 64 || `RAPT_L1I_LEN != 6 || Ways != 4)
      $fatal(1, "test requires 16 KiB / four-way / 64 B default geometry");
    // Four tags for every set, both refill sectors: occupy all 16 KiB.
    for (int line = 0; line < 256; line++) begin
      fetch('h80000000 + 64 * line, 0);
      fetch('h80000020 + 64 * line, 0);
    end
    for (int word_idx = 0; word_idx < 4096; word_idx++) fetch('h80000000 + 4 * word_idx, 1);
    begin
      int before_reads;
      before_reads = reads;
      fetch('h80004000, 0);
      if (reads == before_reads) $fatal(1, "fifth conflicting I tag did not miss");
      fetch('h80004000, 1);
      // Replacement of set 0 must leave neighboring set 1 resident in all ways.
      for (int way = 0; way < 4; way++) fetch('h80000040 + 4096 * way, 1);
      @(negedge clock);
      ifu_l1i.invalid = 1;
    ...
    $display(
        "PASS: L1I RV%0d PA%0d compact tags, 16 KiB capacity, replacement, invalidate/refill, invalid aliases",
