...
#include <cstdio>
#include <cstdlib>
#include "Vrapt_fpu_divsqrt.h"
#include "verilated.h"
extern "C" {
#include "softfloat.h"
}
static Vrapt_fpu_divsqrt dut;
...
static void check(uint64_t a,uint64_t b,bool d,bool div,int rm){
 ...
 uint64_t expected;
 if(d){float64_t x{a},y{b};expected=(div?f64_div(x,y):f64_sqrt(x)).v;}
 else{float32_t x{uint32_t(a)},y{uint32_t(b)};expected=(div?f32_div(x,y):f32_sqrt(x)).v;}
 ...
 dut.src_is_double=d;dut.dst_is_double=d;dut.divide=div;dut.sqrt=!div;dut.rounding_mode=rm;dut.valid=1;tick();dut.valid=0;
 int cycles=0;while(!dut.result_valid && cycles++<200)tick();
 total++;
 if(!dut.result_valid || dut.result!=expected || dut.flags!=flags){
  if(fail++<16)std::printf("FAIL d=%d div=%d rm=%d a=%016llx b=%016llx expected=%016llx/%02x actual=%016llx/%02x valid=%d\n",d,div,rm,(unsigned long long)a,(unsigned long long)b,(unsigned long long)expected,flags,(unsigned long long)dut.result,unsigned(dut.flags),int(dut.result_valid));
 }
 tick();
}
int main(int argc,char**argv){
 ...
 for(int d=0;d<2;d++)for(int div=0;div<2;div++)for(int rm=0;rm<5;rm++){
 ...
 std::printf("SoftFloat divsqrt TOTAL=%d FAILS=%d seed=243f6a8885a308d3\n",total,fail);dut.final();return fail?1:0;
}
