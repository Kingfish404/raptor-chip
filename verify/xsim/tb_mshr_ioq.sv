...
module tb_mshr_ioq;
  ...
  task automatic await_load(input logic [XLEN-1:0] addr);
    ...
    if (!exu_lsu.rvalid || exu_lsu.raddr != addr)
      $fatal(1,"wrong replay/next load: expected=%h valid=%b actual=%h wait=%b",addr,exu_lsu.rvalid,exu_lsu.raddr,dut.ioq_miss_wait);
  endtask
  initial begin
    init_ioq_inputs(1);
    tick(3); reset=0;
    ...
    tick(1); disp.accept[0]=0; disp.accept[1]=0;
    await_load(XLEN'('h80000000));
    exu_lsu.rmiss=1; tick(1); exu_lsu.rmiss=0;
    ...
    exu_lsu.miss_wake=1; tick(1); exu_lsu.miss_wake=0;
    await_load(XLEN'('h80000000));
    // A different MSHR can finish on the same cycle this request is parked.
    exu_lsu.rmiss=1; exu_lsu.miss_wake=1; tick(1);
    exu_lsu.rmiss=0; exu_lsu.miss_wake=0;
    ...
    await_load(XLEN'('h80000100));
    exu_lsu.rready=1; exu_lsu.rdata=13; tick(1); exu_lsu.rready=0;
    check(exu_lsu.rvalid, "response inserted an unnecessary request bubble");
    await_load(XLEN'('h80000000));
    exu_lsu.rready=1; exu_lsu.rdata=12; tick(1); exu_lsu.rready=0;
    for(int n=0;n<30 && dut.ioq_valid!=0;n++) tick(1);
    check(dut.ioq_valid==0,"replays failed to retire");
    cmu_bcast.flush_pipe=1; tick(1); cmu_bcast.flush_pipe=0;
    check(dut.ioq_miss_wait==0,"flush retained waiters");
    $display("PASS: IOQ MSHR RV%0d release, younger progress, replay, simultaneous wake, flush",XLEN);
    $finish;
  end
endmodule
