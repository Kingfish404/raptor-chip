package rapt_chip_pkg;
  ...
  class chip_config extends uvm_object;
    ...
    string mutation="";
    function new(string name = "chip_config");
      super.new(name);
    endfunction
  ...
  class chip_sample extends uvm_sequence_item;
    ...
    bit [LANES-1:0] wstrb;
    function new(string name = "chip_sample");
      super.new(name);
    endfunction
  endclass
  ...
  class chip_monitor extends uvm_monitor;
    ...
    task run_phase(uvm_phase phase);
      forever begin
        ...
        @(posedge vif.clock);
        s=new();
        s.arcache=vif.arcache;
        s.awcache=vif.awcache;
        s.reset=vif.reset;
        ...
        if (!mutated && !s.reset) begin
          if (mutation == "rid" && s.rvalid && s.rready) begin
            s.rid ^= 4'hf;
            mutated = 1;
          end
          if (mutation == "rlast" && s.rvalid && s.rready) begin
            s.rlast ^= 1;
            mutated = 1;
          end
          if (mutation == "rdata" && s.rvalid && s.rready) begin
            s.rdata ^= 1;
            mutated = 1;
          end
          if (mutation == "wlast" && s.wvalid && s.wready) begin
            s.wlast ^= 1;
            mutated = 1;
          end
        end
        if (trace_bus && !s.reset) begin
          if (s.arvalid && s.arready)
            `uvm_info("TRACE_AR", $sformatf(
                      "id=%h addr=%h len=%0d size=%0d", s.arid, s.araddr, s.arlen, s.arsize),
                      UVM_NONE)
          if (s.rvalid && s.rready)
            `uvm_info("TRACE_R", $sformatf(
                      "id=%h data=%h last=%b resp=%h", s.rid, s.rdata, s.rlast, s.rresp), UVM_NONE)
          if (s.awvalid && s.awready)
            `uvm_info("TRACE_AW", $sformatf("id=%h addr=%h", s.awid, s.awaddr), UVM_NONE)
          if (s.wvalid && s.wready)
            `uvm_info("TRACE_W", $sformatf("data=%h strb=%h last=%b", s.wdata, s.wstrb, s.wlast),
                      UVM_NONE)
          if (s.bvalid && s.bready)
            `uvm_info("TRACE_B", $sformatf("id=%h resp=%h", s.bid, s.bresp), UVM_NONE)
        end
        ap.write(s);
      end
    endtask
    ...
    task run_phase(uvm_phase phase);
      ...
      forever begin
        ...
        if (vif.reset) begin
          ...
          if (vif.wvalid && vif.wready) write_beats.push_back('{vif.wdata, vif.wstrb, vif.wlast});
          if (write_beats.size() != 0 && writes.size() != 0) begin
            ...
            if (!cfg.fault_address(a.addr))
              for (int i = 0; i < LANES; i++)
              if (beat.strb[i]) mem[base+64'(i)] = beat.data[8*i+:8];
            if (a.beat == int'(a.len))
        ...
        begin
          ...
          if (vif.reset) begin
            ...
            if (!vif.rvalid && reads.size() != 0 && ready_now()) begin
              ...
              if (reads[i].id == reads[chosen].id) begin
                chosen = i;
                break;
              end
              active_read=chosen;
              ...
              vif.rvalid=1;
              vif.rid=a.id;
              vif.rdata=read_word(a.addr);
              vif.rlast=(a.beat==int'(a.len));
              vif.rresp=cfg.fault_address(a.addr)?2:0;
            end
            if (!vif.bvalid && responses.size() != 0 && ready_now()) begin
              vif.bvalid=1;
              vif.bid=responses[0][5:2];
              vif.bresp=responses[0][1:0];
            end
          end
        end
      end
  ...
  class chip_scoreboard extends uvm_scoreboard;
    ...
    function void write(chip_sample s);
      ...
      previous = s;
      if ((s.arvalid && !s.arready) || (s.awvalid && !s.awready) || (s.wvalid && !s.wready))
        stalls++;
      ...
      if (s.arvalid && s.arready) begin
        ...
        reads.push_back('{s.araddr, s.arid, s.arlen, s.arsize, s.arburst, 0});
        read_count++;
      ...
      if (s.awvalid && s.awready) begin
        ...
        writes.push_back('{s.awaddr, s.awid, s.awlen, s.awsize, s.awburst, 0});
        write_count++;
      end
      ...
      if (s.rvalid && s.rready) begin
        ...
        if (idx < 0) `uvm_error("R", "unsolicited read response")
          ...
          if (s.rresp != (cfg.fault_address(a.addr) ? 2 : 0))
            `uvm_error("RRESP", "read error response mismatch")
          if (s.rresp != 0) read_errors++;
          if (s.rlast) reads.delete(idx);
          else reads[idx].beat++;
        end
      end
        ...
        begin
          ...
          if (a.addr == 'h10000000) begin
            if (beat.strb != LANES'('hf) || beat.data[31:0] != 32'('h600d0000 + signatures))
              `uvm_error("SIGNATURE", $sformatf(
                         "stage %0d got %h strobe %h", signatures, beat.data, beat.strb))
            signatures++;
    ...
    function void check_phase(uvm_phase phase);
      if (!done || signatures != int'(cfg.expected_stages) || read_count == 0 || write_count == 0)
        `uvm_error(
            "COVERAGE", $sformatf(
            "done=%b signatures=%0d reads=%0d writes=%0d", done, signatures, read_count, write_count
            ))
      if (cfg.scenario == "debug" && cfg.debug_checks != 5)
        `uvm_error("DEBUG_COVERAGE", "debug checks incomplete")
      if (cfg.scenario == "irq" && cfg.irq_events != 4)
        `uvm_error("IRQ_COVERAGE", "interrupt pin stimulus incomplete")
      if (cfg.scenario == "external" && cfg.external_events != 2)
        `uvm_error("EXTERNAL_COVERAGE", "device write notifications incomplete")
      if (!cfg.control_done) `uvm_error("CONTROL", "stimulus sequence did not complete")
      if (cfg.scenario == "reset" && cfg.reset_epochs < 3)
        `uvm_error("RESET", "reset-under-load not exercised")
      if (cfg.scenario == "faults" && (read_errors < 2 || write_errors < 1))
        `uvm_error("FAULT_COVERAGE", "bus faults not exercised")
      if(writes.size()!=0 || responses.size()!=0 || write_beats.size()!=0 || reads.size()!=0)
        `uvm_error("DRAIN", "accepted transactions not drained")
      `uvm_info("CHIP_COVERAGE", $sformatf(
                "reads=%0d writes=%0d stalls=%0d signatures=%0d bursts=%0d outstanding=%0d overlap=%0d read_errors=%0d write_errors=%0d reset_epochs=%0d debug=%0d irq=%0d external=%0d reordered=%0d early_writes=%0d",
                read_count,
                write_count,
                stalls,
                signatures,
  ...
  class chip_test extends uvm_test;
    ...
    task run_phase(uvm_phase phase);
      ...
      `uvm_fatal(
          "TIMEOUT",
          $sformatf(
              "case=%s done=%b stages=%0d command=%0d control_done=%b reads=%0d aw=%0d w=%0d b=%0d model_reads=%0d model_aw=%0d model_w=%0d model_b=%0d AR=%b%b:%h R=%b%b:%h W=%b%b B=%b%b",
              cfg.scenario, env.scoreboard.done, env.scoreboard.signatures, cfg.command,
              cfg.control_done, env.scoreboard.reads.size(), env.scoreboard.writes.size(),
              env.scoreboard.write_beats.size(), env.scoreboard.responses.size(),
              env.memory.reads.size(), env.memory.writes.size(), env.memory.write_beats.size(),
              env.memory.responses.size(), vif.arvalid, vif.arready, vif.araddr, vif.rvalid,
              vif.rready, vif.rid, vif.wvalid, vif.wready, vif.bvalid, vif.bready))
    endtask
