#!/usr/bin/env python3
"""Generate RAgentBench summary reports from benchmark log files."""
...
MODE_ORDER = {"tools": 0, "rag": 1, "workflow": 2}
...
def parse_log(path):
    ...
    with path.open("r", encoding="utf-8", errors="replace") as log_file:
        for raw_line in log_file:
            line = raw_line.strip()
            if line.startswith("RAGENTBENCH_INFO"):
                parsed["info"].update(parse_fields(line))
            elif line.startswith("RAGENTBENCH_CONFIG"):
                parsed["config"].update(parse_fields(line))
            elif line.startswith("RAGENTBENCH_RESULT"):
                parsed["results"].append(parse_fields(line))
            elif line.startswith("RAGENTBENCH_SCORE"):
                parsed["scores"].append(parse_fields(line))
            elif line.startswith("RAGENTBENCH_END"):
...
def main():
    parser = argparse.ArgumentParser(description="Generate RAgentBench reports from benchmark logs")
    parser.add_argument("--log-dir", required=True, type=Path)
    parser.add_argument("--markdown", required=True, type=Path)
    parser.add_argument("--csv", required=True, type=Path)
    parser.add_argument("--json", required=True, type=Path)
    parser.add_argument("--suite", default="RAgentBench")
    ...
    if not mode_rows:
        raise SystemExit(f"no RAGENTBENCH_SCORE lines found in {args.log_dir}")
