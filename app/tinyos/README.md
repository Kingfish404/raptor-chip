...
`egos-cli-nsim` boots the upstream egos QEMU image path without an egos patch: `tools/egos.bin` is loaded as the boot image and `tools/disk.img` is attached through sim's QEMU-compatible SDHCI model at `0x40000000` with PCI ECAM at `0x30008000`. `egos-cli-nemu` uses the same image pair through NEMU's optional `--sdcard` path.
