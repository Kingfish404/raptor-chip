...
rapt (CPU) --AXI--> axi2rnp --rnp pins--> rnp2axi --AXI4--> rapt_tb_mem
            (inside rng_chip / soc_pad)   (hdl/perip/wrap_rnp.sv)
```
...
## Memory map (kept in sync with `sim/include/common.h`)
...
| `+MROM=<bin>`                        | reset trampoline → MROM `0x2000_0000` (default: sim `mrom-data.bin`) |
...
| `MROM`            | trampoline `.bin` → MROM (default: sim `mrom-data.bin`)        |
...
The rnp data path is 32-bit, so the harness is **RV32 only**. `ISA64=1` is rejected with an error; use the AXI-based `sim` flow for RV64.
