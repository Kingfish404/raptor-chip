...
static void check_bound(IOMap *map, paddr_t addr, bool is_read)
{
  if (map == NULL)
  {
    if (cpu.sr[CSR_MTVEC] != 0 && (cpu.priv == PRV_M || cpu.priv == PRV_S || cpu.priv == PRV_U))
    {
  ...
  else
  {
    Assert(addr <= map->high && addr >= map->low,
           "map.c > check_bound: address (" FMT_PADDR ") is out of bound {%s} [" FMT_PADDR ", " FMT_PADDR "] at pc = " FMT_WORD,
