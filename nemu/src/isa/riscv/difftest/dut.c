...
bool isa_difftest_checkregs(CPU_state *ref_r, vaddr_t pc)
{
  ...
  for (int i = 0; i < 32; i++)
  {
    if (cpu.fpr[i] != ref_r->fpr[i])
    {
      printf(ANSI_FMT("Difftest:      fpr[%d]: %016" PRIx64 ", ref: %016" PRIx64 "\n", ANSI_FG_RED),
             i, cpu.fpr[i], ref_r->fpr[i]);
      is_same = false;
    }
  }
  if (cpu.sr[CSR_FCSR] != ref_r->sr[CSR_FCSR])
  {
    printf(ANSI_FMT("Difftest:        fcsr: " FMT_WORD ", ref: " FMT_WORD "\n", ANSI_FG_RED),
           cpu.sr[CSR_FCSR], ref_r->sr[CSR_FCSR]);
    is_same = false;
  }
