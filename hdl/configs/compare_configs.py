#!/usr/bin/env python3
"""Compare Raptor config presets under hdl/configs/*/rapt_config.svh.

Usage:
    python3 hdl/configs/compare_configs.py
    python3 hdl/configs/compare_configs.py --format markdown
    python3 hdl/configs/compare_configs.py --format plain
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare hdl/configs/*/rapt_config.svh")
    parser.add_argument(
        "--format",
    ...
    if not cfg_files:
        raise SystemExit("No configs found under hdl/configs/*/rapt_config.svh")
